...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        const [all, bal] = await Promise.all([getAllArticles(address), getAuthorBalance(address, address)]);
  ...
  const totalEarnings = articles.reduce((s, a) => s + parseFloat(a.price) * a.read_count, 0);
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main style={{ maxWidth: 1000, margin: "0 auto", padding: "48px 24px" }}>
        ...
        <div className="page-enter mb-9">
          ...
          <p className="text-[13px] text-muted-foreground">Your earnings, reads, and published articles.</p>
        </div>

        {!address ? (
          <Card className="p-12 text-center">
            <CardHeader>
              <CardTitle className="font-serif text-[22px] font-bold text-foreground">Connect to see your dashboard</CardTitle>
              <CardDescription className="text-[13px] text-muted-foreground">Track earnings and manage your articles</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <>
            ...
            <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
              {[
                ...
                { label: "Est. Earned",    value: totalEarnings.toFixed(2),              suffix: " XLM" },
                { label: "Pending",        value: parseFloat(balance).toFixed(2),        suffix: " XLM" },
              ].map((s) => (
            ...
            <Card className={cn(
              ...
              parseFloat(balance) > 0 ? "border-green-500/30 bg-green-500/5" : "bg-muted/30"
            )}>
              <div className="flex-1">
                ...
                <div className={cn(
                  ...
                  parseFloat(balance) > 0 ? "text-green-600 dark:text-green-400" : "text-muted-foreground"
                )}>
              ...
              <div className="flex w-full flex-col gap-2.5 sm:w-auto sm:min-w-[200px]">
                ...
                >
                  {withdrawing ? "Signing in Freighter…" : parseFloat(balance) > 0 ? `Withdraw ${balance} XLM` : "Nothing to withdraw"}
                </Button>
                {error && <p className="text-center text-[12px] text-destructive sm:text-right">{error}</p>}
                {txHash && (
            ...
            {/* Article table */}
            <div>
              <div style={{ borderTop: "2px solid var(--ink)", paddingTop: 12, marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <h2 className="font-serif" style={{ fontSize: 20, fontWeight: 700, color: "var(--ink)" }}>Your Articles</h2>
                <a href="/write"><button className="btn-ghost">+ New Article</button></a>
              </div>
              ...
              {!loading && articles.length === 0 && (
                <div className="card" style={{ padding: "36px 28px", textAlign: "center" }}>
                  <p className="font-serif" style={{ fontSize: 18, fontWeight: 700, marginBottom: 8 }}>No articles yet</p>
                  <p style={{ fontSize: 13, color: "var(--ink-muted)", marginBottom: 20 }}>
                    Start writing and earn XLM directly from readers
                  </p>
                  <a href="/write"><button className="btn-primary">Write First Article</button></a>
                </div>
              )}
                ...
                <div style={{ border: "1px solid var(--rule)", borderRadius: 4, overflow: "hidden" }}>
                  ...
                  <div style={{
                    ...
                    {["Title", "Price", "Reads", "Earned", ""].map((h) => (
                      <span key={h} style={{ fontSize: 10, fontWeight: 700, color: "var(--ink-muted)", textTransform: "uppercase", letterSpacing: "0.08em" }}>{h}</span>
                    ))}
                  </div>
                  {articles.map((article, i) => (
                    ...
                    >
                      <p className="font-serif" style={{ fontSize: 14, fontWeight: 600, color: "var(--ink)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", paddingRight: 16 }}>
                        {article.title}
                      </p>
                      <span className="font-mono" style={{ fontSize: 12, color: "var(--ink-muted)" }}>{article.price} XLM</span>
                      <span style={{ fontSize: 13, color: "var(--ink-soft)" }}>{article.read_count}</span>
                      <span className="font-mono" style={{ fontSize: 12, color: "var(--green)", fontWeight: 500 }}>
                        {(parseFloat(article.price) * article.read_count).toFixed(3)} XLM
                      </span>
